...
void Move(int rx, int ry, int bx, int by, int cnt, int dir, string s) {
	if (cnt > result)
		return;
	if (cnt > 10)
		return;
	...
	for (int i = 0;i < 4;i++) {
		if (i == dir || i == inverse_dir(dir))
			continue;
		Move(nrx, nry, nbx, nby, cnt + 1, i,s+DirToChar(i));
	}
}
...
int main() {
	...
	for (int i = 0;i < 4;i++) {
		Move(Red.first, Red.second, Blue.first, Blue.second, 1, i,DirToChar(i));
	}
